...
echo 'Enter the binary name of your program.'
echo 'This is the name of the binary flie installed to /usr/bin:'
read bin_name
if [ -z "$bin_name" ]; then
...
echo 'Enter a readable name of your program ':
read prog_name
if [ -z "$prog_name" ]; then
    echo 'Please enter a valid name.'
    return
...
for f in `ls ./configure.ac ./src/*.c ./src/*.am ./data/*.am 
./data/*.in ./data/ui/*.am ./data/ui/*.in`; do
    sed -e s/template_/"$bin_name_"
    / -e s/template/"$bin_name"/ -e s/Template/"$prog_name"
    / -e "s/Template Description/$prog_desc/" "$f" > "$f".tmp
    mv "$f".tmp "$f"
done
